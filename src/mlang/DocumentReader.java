...
public class DocumentReader extends Reader {
	...
	static class DegatedCharSequence implements CharSequence {

		private final DocumentReader reader;
		
		public DegatedCharSequence(DocumentReader reader) {
			this.reader = reader;
		}

		@Override
		public int length() {
			return reader.fCharSequence.length();
		}

		@Override
		public char charAt(int index) {
			return reader.fCharSequence.charAt(index);
		}

		@Override
		public CharSequence subSequence(int start, int end) {
			return reader.fCharSequence.subSequence(start, end);
		}
	}
	 ...
	 * @param document the document
	 */
	public DocumentReader(IDocument document, int offset, int range) {
		Assert.isNotNull(document);
		fDocument= document;
		fCharSequence= new DocumentCharSequence(fDocument);
		fDocument.addDocumentListener(fDocumentListener);
		fOffset = offset;
		fLength= offset + range;
	}
	...
	public DocumentReader(DocumentReader reader, int offset, int range) {
		fDocument= null;
		fCharSequence= new DegatedCharSequence(reader);
		fOffset = offset;
		fLength= offset + range;
	}
